...
## 2.1 Геометрия плоскости

После генерации случайных направлений `d1`, `d2` выполняется ортогонализация Грам-Шмидта:
`d2 ← d2 - (<d2,d1>/<d1,d1>) * d1`, затем перенормировка filter-wise.
Это гарантирует ортогональность осей плоскости и устраняет неоднозначность интерпретации
